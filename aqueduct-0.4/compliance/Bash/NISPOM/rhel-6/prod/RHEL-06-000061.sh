...
if [ $? -ne 0 ]; then
	...
	R5='auth        required      pam_faillock.so preauth silent audit deny=3 even_deny_root root_unlock_time=900 fail_interval=900'
	...
	R5='auth        [default=die] pam_faillock.so authfail deny=3 fail_interval=900'
	add_entry_after "$R1" "$R5" $PKG_CONFIG
	add_entry_after "$R1" "$R5" $PKG_CONFIG2

	R1='auth        requisite     pam_succeed_if.so uid >= 500 quiet'
	R5='auth        required      pam_faillock.so authsucc deny=3 fail_interval=900'
	add_entry_after "$R1" "$R5" $PKG_CONFIG
	add_entry_after "$R1" "$R5" $PKG_CONFIG2
...
# DEbug
cp -f /usr/libexec/aqueduct/NISPOM/rhel-6/scripts/system-auth /etc/pam.d/
cp -f /usr/libexec/aqueduct/NISPOM/rhel-6/scripts/password-auth /etc/pam.d/
